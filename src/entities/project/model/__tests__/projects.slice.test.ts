...
// Define a type for the root state, specific to this test file
...
describe('projectsSlice + thunks', () => {
	...
	it('fetchProjects populates state from IndexedDB', async () => {
		// Pre-populate the DB via the repository
		const now = Date.now();
		...
		// Check for sorting by updatedAt (desc) from the slice config
